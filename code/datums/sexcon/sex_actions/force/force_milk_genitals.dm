...
/datum/sex_action/force_milk_genitals/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	if(target.getorganslot(ORGAN_SLOT_PENIS))
		user.sexcon_action_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective()]隔着[user.get_active_held_item()]撸弄[target]的肉棒……"))
	else
		user.sexcon_action_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective()]隔着[user.get_active_held_item()]抚弄[target]的阴部……"))
