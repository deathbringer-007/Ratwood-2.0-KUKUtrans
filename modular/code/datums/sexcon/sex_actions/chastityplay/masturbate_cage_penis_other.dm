...
/datum/sex_action/chastityplay/masturbate_cage_penis_other/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
    user.sexcon_action_message(user.sexcon.spanify_force("[user][user.sexcon.get_generic_force_adjective()]以稳定的力道套弄[target]的[get_chastity_device_name(target)]，每一次拉动都只让[target]的阴茎徒劳地顶向栅栏......"))
