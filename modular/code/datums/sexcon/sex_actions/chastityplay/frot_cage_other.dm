...
/datum/sex_action/chastityplay/frot_cage_other/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
    ...
    if(HAS_TRAIT(user, TRAIT_CHASTITY_SPIKED))
        user.sexcon_action_message(user.sexcon.spanify_force("[user][user.sexcon.get_generic_force_adjective()]拖着[target]的阴茎划过带刺的[get_chastity_device_name(user)]外表面，每一下都留下新的刺痛..."))
        ...
        return
    user.sexcon_action_message(user.sexcon.spanify_force("[user][user.sexcon.get_generic_force_adjective()]让[target]的阴茎沿着[get_chastity_device_name(user)]栅条来回磨蹭，每一次划过都带起轻微的金属刮响..."))
    user.sexcon.outercourse_noise(target, TRUE)
